...
4.  **「このエンドポイントのリクエスト・ペイロードを構成」** をチェックします。
    他のチェックボックスはチェックせずに **「続行」** をクリックします。

5.  **リクエストの構成** パネルが表示されます。
    **「リクエスト・ペイロード書式を選択」** で **「JSON サンプル」** が選択されていることを確認します。

6.  REST API リクエストとして受け入れる JSON のサンプルを入力します。
...
![スクリーンショット: REST トリガー追加直後の統合キャンバス](./images/integration_canvas.png)
