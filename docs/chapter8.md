...
前章の「7. 統合のテスト実行」では、Integration Canvas の **「テスト」** ボタンをクリックすることで、自動的に統合がアクティブ化されていました。
Oracle Integration の外部から統合の実行をテストや、本番運用を開始する場合には、明示的に統合をアクティブ化する必要があります。

1.  本チュートリアルのために作成したプロジェクトを開きます。

2.  作成した統合のメニュー・ボタンをクリックし、**「アクティブ化」** を選択します。

3.  「統合のアクティブ化」パネルが表示されたら、**「アクティブ化」** ボタンをクリックします。

    ![スクリーンショット: 統合のアクティブ化](images/activate_button.png)

4.  アクティブ化が成功したことを伝えるメッセージが表示されたら、サービス・コンソール右上にある **「リフレッシュ」** ボタンをクリックします。
...
2.  統合の実行画面が表示されたら、 **「エンドポイント・メタデータ」** ボタンをクリックします。
...
```sh
https://<your-instance>/ic/api/integration/v2/flows/rest/project/<project-id>/<integration-id>/<version>/SalesHistory/export
```

## 8.4 curl による統合の実行

本チュートリアルでは、OCI コンソールの Cloud Shell 上で curl コマンドを使用して、統合を実行する手順を説明します。
Cloud Shell は、OCI コンソールの画面右上にある **「開発者ツール」** メニューをクリックし、 **「Cloud Shell」** を選択すると起動されます。

今回作成した統合は、POST メソッドでレスポンス・ペイロードとして JSON を受け取ります。
...
Cloud Shell が起動されたら、次の例のように curl コマンドを実行します。
...
- `https://<your-instance>/...` は、8.3 で取得した Endpoint URL と置き換えてください
- `user@example.com` と `password` は、Oracle Integration にログインしたユーザー名とパスワードに置き換えてください

### 出力例

Oracle Integration によってリクエストが受信されると、HTTP ステータスとして `202 Accepete` が返ってきます。
...
他のステータスが返ってきた場合は、8.7 トラブル・シューティングを確認してください。

## 8.5 統合の実行結果の確認

curl コマンドの実行に成功したら、統合の実行画面の **「インスタンスのトラッキング」** ボタンをクリックします。
これにより、プロジェクトの **「監視」** ページが表示され、直近1時間の統合の実行結果がリスト形式で表示されます。

![スクリーンショット: インスタンスのトラッキング](images/run_success.png)

ステータスが **「成功」** と表示されていれば、統合の実行が成功したことを表しています。

## 8.6 統合の REST エンドポイントの認証

Oracle Integration の REST エンドポイントは認証が必要です。
本チュートリアルでは、REST Adapter トリガー接続 (REST Trigger Conn) を作成した際に、セキュリティ・ポリシーとして「OAuth 2.0 Or Basic Authentication」を選択しました。
...
また、REST Adapter を使用することで、統合を外部から呼び出せる API として公開できることを確認しました。
